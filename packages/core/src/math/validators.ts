 ...
 */
export function validateVoteParameters(
  weight: BigIntString,
  stake: BigIntString
): void {
  if (!weight || !stake) {
 ...
 */
export function validateProposalParameters(
 ...
 */
export function validateStakeTransaction(amount: BigIntString): void {
 ...
 */
export function estimateTotalVotingCost(votes: BigIntString[]): BigIntString {
  const total = votes.reduce((sum, vote) => {
    return sum + BigInt(calculateVoteCost(vote));
  }, BigInt(0));
  return total.toString() as BigIntString;
}
