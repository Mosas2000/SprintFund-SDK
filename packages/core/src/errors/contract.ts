...
const ERROR_MESSAGES: Record<ContractErrorCode, string> = {
  [ContractErrorCode.ERR_INVALID_PROPOSAL_ID]:
    'Proposal ID does not exist or is invalid',
  ...
  [ContractErrorCode.ERR_INVALID_VOTE_WEIGHT]:
    'Vote weight is invalid or exceeds maximum',
  ...
  [ContractErrorCode.ERR_INVALID_PARAMETERS]:
    'Invalid parameters provided to contract function',
...
const ERROR_REMEDIATIONS: Record<ContractErrorCode, string> = {
  [ContractErrorCode.ERR_INVALID_PROPOSAL_ID]:
    'Verify the proposal ID is correct and try again',
  [ContractErrorCode.ERR_PROPOSAL_INACTIVE]:
    'Wait until the proposal becomes active or check proposal status',
  [ContractErrorCode.ERR_INSUFFICIENT_STAKE]:
    'Stake more tokens in the protocol before voting',
  [ContractErrorCode.ERR_INVALID_VOTE_WEIGHT]:
    'Reduce the vote weight or check your voting power limit',
  [ContractErrorCode.ERR_DUPLICATE_VOTE]:
    'You have already voted on this proposal',
  [ContractErrorCode.ERR_PROPOSAL_NOT_ENDED]:
    'Wait for the proposal voting period to end',
  [ContractErrorCode.ERR_UNAUTHORIZED]:
    'Ensure you have the required permissions for this action',
  [ContractErrorCode.ERR_INVALID_PARAMETERS]:
    'Check the contract function parameters and try again',
  [ContractErrorCode.ERR_INSUFFICIENT_FUNDS]:
    'Ensure you have enough funds for this operation',
  [ContractErrorCode.ERR_INVALID_ADDRESS]:
...
export function getContractErrorRemediation(code: ContractErrorCode): string {
  return (
    ERROR_REMEDIATIONS[code] ||
